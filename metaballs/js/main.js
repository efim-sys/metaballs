...
const MISSILE_DELAY = 1200; // Lenkraketenabklingzeit in Millisekunden
...
const LASER_DELAY = 650; // Laserabklingzeit in Millisekunden
const UFO_VELOCITY = 150; // UFO-Geschwindigkeit in Pixel pro Sekunde
const UFO_PROP_SPAWN = 0.75; // Wahrscheinlichkeit für Erscheinen eines UFOs pro Sekunde
const UFO_PROP_LASER = 0.35; // Wahrscheinlichkeit für Erscheinen eines LASERs pro Sekunde
...
export class Scene {
    ...
    update() {
        ...
        if (this.keys.has(KEY_SHOOT_LASER) && this.now - this.lastLaser > LASER_DELAY) {
            this.lastLaser = this.now;
            this.shootLaser(this.ship, this.ship.rotation)
        }

        if (this.keys.has(KEY_SHOOT_ROCKET) && this.now - this.lastMissile > MISSILE_DELAY && this.target) {
        ...
        for (var obj of this.objects) {
            switch (obj.id) {
                case 'ship': {
                    obj.wormhole();
                    var boosting = this.keys.has(KEY_BOOST);
                    if (obj.content) {
                        obj.content.getElementById("flame").setAttribute('opacity', 1 * boosting);
						console.log("Boosting = "+stringlify(boosting));
                    }
